...
for sheet_name, df in excel_file.items():
    if df.shape[1] >= 11:
        ...
        # VALIDATION: Keep rows where Power Time OR Run Time OR Stops is NOT empty
        # This skips rows that only have Machine/Quality (Template rows)
        day_mask = day_raw.iloc[:, [2, 3, 4]].notna().any(axis=1)
        day_valid = day_raw[day_mask].copy()

        if not day_valid.empty:
            day_p = process_shift(day_valid, 'Day', DAY_LIMIT, sheet_name)
