...
# --- 5e. JOB WORK EXCEL GENERATION (SIDE-BY-SIDE BLOCKS) ---
jobwork_file_path = os.path.join(base_path, 'JOBWORK.xlsx')

if os.path.exists(beam_book_file):
    df_jw_raw = pd.read_excel(beam_book_file)
    
    if 'JOB' in df_jw_raw.columns:
        df_jw_only = df_jw_raw[df_jw_raw['JOB'].notna()].copy()
        
        if not df_jw_only.empty:
            with pd.ExcelWriter(jobwork_file_path, engine='xlsxwriter') as jw_writer:
                workbook = jw_writer.book
                
                # Define Professional Formats
                header_fmt = workbook.add_format({'bold': True, 'bg_color': '#FFEB9C', 'border': 1, 'align': 'center'})
                total_fmt = workbook.add_format({'bold': True, 'bg_color': '#F2F2F2', 'top': 2, 'bottom': 1, 'align': 'center'})
                data_fmt = workbook.add_format({'align': 'center', 'border': 1})

                # 1. Group by Job Party Name (Each party gets a sheet)
                for party_name, party_data in df_jw_only.groupby('JOB'):
                    sheet_name = str(party_name)[:31]
                    # We write to a dummy dataframe first to initialize the sheet
                    pd.DataFrame().to_excel(jw_writer, sheet_name=sheet_name)
                    ws = jw_writer.sheets[sheet_name]
                    
                    current_col = 0 # Starting column for the first Quality block
                    
                    # 2. Loop through each Quality for this Party
                    for quality_name, qual_data in party_data.groupby('Quality'):
                        # Prepare the specific data for this quality block
                        display_df = qual_data[['Quality', 'Beam No', 'Warp Meter']].copy()
                        display_df.columns = ['QUALITY', 'BEAM NO', 'METERS']
                        
                        # Write Headers manually
                        for col_num, col_name in enumerate(display_df.columns):
                            ws.write(0, current_col + col_num, col_name, header_fmt)
                        
                        # Write Data rows manually
                        row_idx = 1
                        for _, row in display_df.iterrows():
                            ws.write(row_idx, current_col, row['QUALITY'], data_fmt)
                            ws.write(row_idx, current_col + 1, row['BEAM NO'], data_fmt)
                            ws.write(row_idx, current_col + 2, row['METERS'], data_fmt)
                            row_idx += 1
                        
                        # 3. Add the GRAND TOTAL row for THIS Quality
                        grand_total_row = row_idx
                        total_beams = len(display_df)
                        total_meters = display_df['METERS'].sum()
                        
                        ws.write(grand_total_row, current_col, "GRAND TOTAL", total_fmt)
                        ws.write(grand_total_row, current_col + 1, total_beams, total_fmt)
                        ws.write(grand_total_row, current_col + 2, total_meters, total_fmt)
                        
                        # Set Column Widths for this block
                        ws.set_column(current_col, current_col, 18)     # Quality
                        ws.set_column(current_col + 1, current_col + 1, 12) # Beam No
                        ws.set_column(current_col + 2, current_col + 2, 12) # Meters
                        
                        # Add a small gap (blank column) before the next Quality
                        ws.set_column(current_col + 3, current_col + 3, 3) 
                        
                        # Move the starting point for the next block to the right (4 columns over)
                        current_col += 4 

            print(f"JOBWORK Excel generated with side-by-side format for {df_jw_only['JOB'].nunique()} parties.")
        else:
            print("No Job Work entries found.")
    else:
